 ...
 */
SymbolTableEntry *create_SymbolTableEntry(char *name, SymList_T argument_list,
                                          unsigned int scope,
...
void SymTable_append_addresslist(SymbolTableEntry *entry, unsigned int label) {
    LabelList *prev = entry->address_list;
    if (!prev) {
        entry->address_list = create_list(label);
        return;
    }
    while (prev->next) prev = prev->next;
    prev->next = create_list(label);
}
