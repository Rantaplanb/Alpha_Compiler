...
unsigned int SymTable_getLength(SymTable_T oSymTable) { /* Returns node count. */
    assert(oSymTable);
...
int SymTable_remove(SymTable_T oSymTable, const char *pcKey) {
    ...
    for (curr_node = oSymTable->buckets[index]; curr_node;
         curr_node =
                 curr_node->next) { /* check every node on that bucket. If found,
                                   we need to remove the node, update
...
int SymTable_contains(SymTable_T oSymTable, const char *pcKey,
    ...
    for (curr_node = oSymTable->buckets[index]; curr_node;
         curr_node = curr_node->next)
        if (ms_compare(curr_node->key, pcKey) == 0 && scope == curr_node->entry->scope && curr_node->entry->isActive)
...
SymbolTableEntry *SymTable_get(SymTable_T oSymTable, const char *pcKey) {
    ...
    for (curr_node = oSymTable->buckets[index]; curr_node;
         curr_node = curr_node->next)
        if (ms_compare(curr_node->key, pcKey) == 0 && curr_node->entry->isActive == 1)
...
SymbolTableEntry *SymTable_get_entry_at_scope(SymTable_T oSymTable,
    ...
    assert(oSymTable);
    if (scope >= oSymTable->scopes_size)
        return NULL;
...
SymbolTableEntry *SymTable_get_closest_scope(SymTable_T oSymTable, const char *pcKey, unsigned int scope) {
    ...
    for (index = scope; index > 0; index--) {
        ...
        while (curr_node) {
            if (ms_compare(curr_node->entry->name, pcKey) == 0 && curr_node->entry->isActive == 1)
                return curr_node->entry; /* If we found a node with a matching
                                        string we return its pvValue */
            curr_node = curr_node->next;
        }
...
int SymTable_open_func(SymTable_T oSymTable, SymbolTableEntry *entry, int max_scope) {
    int i;
    list_entry_T *curr_node;
    assert(oSymTable);
    assert(entry);
    ...
    for (; i < max_scope; i++) {
        while (curr_node != NULL) {
            if (curr_node->entry->func_is_open)
                return 1;
            curr_node = curr_node->next;
        }
