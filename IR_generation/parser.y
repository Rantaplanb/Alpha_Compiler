...
expression
    ...
    | expression ADDITION expression {
        if (IS_DUMMY($1) || IS_DUMMY($3))
            ...
            $$ = handle_expr_op_expr($1, add_opc, $3);
        }
    }
    | expression SUBTRACTION expression {
        if (IS_DUMMY($1) || IS_DUMMY($3))
            ...
            $$ = handle_expr_op_expr($1, sub_opc, $3);
        }
    }
    | expression MULTIPLICATION expression {
        if (IS_DUMMY($1) || IS_DUMMY($3))
            ...
            $$ = handle_expr_op_expr($1, mul_opc, $3);
        }
    }
    | expression DIVISION expression {
        if (IS_DUMMY($1) || IS_DUMMY($3))
            ...
            $$ = handle_expr_op_expr($1, div_opc, $3);
        }
    }
    | expression MODULO expression {
        if (IS_DUMMY($1) || IS_DUMMY($3))
            ...
            $$ = handle_expr_op_expr($1, mod_opc, $3);
        }
    }
    | logical_expression
    | terminal
...
// TODO: maybe assign_expr_e is redundant, just bubble up <expression> ok?
assignment_expression
    : lvalue ASSIGNMENT expression {
        if (IS_DUMMY($1) || IS_DUMMY($3))
            ...
            if ($1->type == table_item_e) {
                emit(tablesetelem_opc, $1->index, $3, $1, yylineno, 0);
...
lvalue
	...
	if (SymTable_get_entry_at_scope(symbol_table, $2, 0) &&
		SymTable_get_entry_at_scope(symbol_table, $2, 0)->type == LIBFUNC_SYMBOL) {
...
object_definition
    : LEFT_SQUARE_BRACKET rev_elist RIGHT_SQUARE_BRACKET {
        if ($2 && IS_DUMMY($2))
            ...
            for (size_t i = 0; $2; $2 = $2->next)
                emit(tablesetelem_opc, result, create_numeric_expr(i++), $2,
                     yylineno, 0);
    ...
    | LEFT_SQUARE_BRACKET indexed RIGHT_SQUARE_BRACKET {
        if (IS_DUMMY($2))
            ...
            emit(tablecreate_opc, result, NULL, NULL, yylineno, 0);
            for (size_t i = 0; $2; $2 = $2->next)
                emit(tablesetelem_opc, $2, $2->index, result, yylineno, 0);
            $$ = result;
        }
    }
...
while_start
    : WHILE {
        $$ = next_quad();
        insert_stack(block_stack, yylineno, LOOP);
    }
    ;
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 1; i < argc; ++i) {
        if (!strcmp(argv[i], "-o")) {
            air_file = fopen(argv[++i], "w");
        } else if (!strcmp(argv[i], "--scopesfile") ||
                   !strcmp(argv[i], "-sf")) {
            scopes_file = fopen(argv[++i], "w");
        } else {
            yyin = fopen(argv[i], "r");
            if (!air_file) {
                char *prefix = strsep(&argv[i], ".");
                char air_file_name[30];
                strncpy(air_file_name, prefix, 30);
                strncat(air_file_name, ".air", 30 - strlen(prefix));
                air_file = fopen(air_file_name, "w");
            }
        }
    }
    if (!air_file)
