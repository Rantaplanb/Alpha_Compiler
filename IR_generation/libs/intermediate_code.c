...
unsigned int get_next_quad_label() { return curr_quad; }
...
SymbolTableEntry *create_temp_symbol() {
    ...
    if (SymTable_contains(symbol_table, name, current_scope)) {
    ...
    SymbolTableEntry *new_symbol = create_SymbolTableEntry(
        name, NULL, current_scope, yylineno, LOCAL_SYMBOL);
    if (!SymTable_get_entry_at_scope(symbol_table, new_symbol->name,
                                     current_scope))
...
void reset_temp_var_counter() {
    ...
    for (i = 0; i < temp_var_counter; i++) {
        sprintf(tmp, "@t%d", i);
        assert(SymTable_remove(symbol_table, tmp));
    }
      */
    temp_var_counter = 0;
    return;
...
Expr *make_call(Expr *lv, Expr *rev_elist, unsigned int line) {
    ...
    emit(call_opc, func, NULL, NULL, line, 0);  // TODO: check if label should
    // be 0
    Expr *result = create_expr(var_e);
    result->sym_value = create_temp_symbol();
